"""
    This code is modified based on https://github.com/NJU-PCALab/OpenVid-1M
"""
...
def download_videos(output_directory:str, video_folder: str, index: int) -> None:

    """only download one subfolder of OpenVid-1M"""

    zip_folder = os.path.join(output_directory, "download")
    video_folder = os.path.join(output_directory, video_folder)
    os.makedirs(zip_folder, exist_ok=True)
    os.makedirs(video_folder, exist_ok=True)
    error_log_path = os.path.join(zip_folder, "download_log.txt")

    url = f"https://huggingface.co/datasets/nkp37/OpenVid-1M/resolve/main/OpenVid_part{index}.zip"
    file_path = os.path.join(zip_folder, f"OpenVid_part{index}.zip")
    if os.path.exists(file_path):
        ...
        try:
            ...
            part_urls = [
                f"https://huggingface.co/datasets/nkp37/OpenVid-1M/resolve/main/OpenVid_part{index}_partaa",
                f"https://huggingface.co/datasets/nkp37/OpenVid-1M/resolve/main/OpenVid_part{index}_partab"
            ]
            ...
            file_path = os.path.join(zip_folder, f"OpenVid_part{index}.zip")
            cat_command = "cat " + os.path.join(zip_folder, f"OpenVid_part{index}_part*") + " > " + file_path
            os.system(cat_command)
    unzip_command = ["unzip", "-j", file_path, "-d", video_folder]
    subprocess.run(unzip_command, check=True)


def download_metadata(output_directory:str) -> None:
    data_folder = os.path.join(output_directory, "metadata")
...
def download_files(output_directory):
    zip_folder = os.path.join(output_directory, "download")
    video_folder = os.path.join(output_directory, "eval")
    os.makedirs(zip_folder, exist_ok=True)
    os.makedirs(video_folder, exist_ok=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_directory', type=str, help='Path to the dataset directory', default="/hpc2hdd/home/yhuang489/OpenVid")
